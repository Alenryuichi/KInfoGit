...
export default function TextPressure({
  ...
  useEffect(() => {
    ...
    if (existingStyle) {
      existingStyle.textContent = `
        ...
        .text-pressure-flex-${styleId} {
          display: flex;
          justify-content: space-between;
        }
        .text-pressure-stroke-${styleId} span {
          position: relative;
          color: ${textColor};
        }
        .text-pressure-stroke-${styleId} span::after {
  ...
  }, [styleId, fontFamily, fontUrl, textColor, strokeColor])

  const dynamicClassName = [
    className,
    flex ? `text-pressure-flex-${styleId}` : '',
    stroke ? `text-pressure-stroke-${styleId}` : ''
  ].filter(Boolean).join(' ')
  ...
  return (
    ...
    >
      <h1
        ref={titleRef}
        className={`text-pressure-title-${styleId} ${dynamicClassName}`}
        style={{
          fontFamily,
