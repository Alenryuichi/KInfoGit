...
>
> 约定：
> - ✅ = 已上线跑通  · 🚧 = 进行中  · 📐 = 已设计未开工  · 💡 = 想法待验证  · 🚫 = 已撤销（前提过时或决策判否）
...
- 🚫 **~~中文/英文分流~~**（2026-04-23 撤销）：原条目前提已过时——实测 62/62 item **均无 `translations` 字段**，且**当前数据已是"英 title + 中 oneLiner"混排形态**（`title_lang=EN=62 / summary_lang=ZH=62` 实测）。再加 EN-only / ZH-only 切换也换不出新信息：点"只看英文"要么藏 summary（丢信息量），要么啥都不变。结论是**现状混排已是最佳形态**，不折腾双列或翻译增强。若未来某天 DeepSeek prompt 升级到生成 `zhAbstract`（300 字中文长摘要）让详情页变成"信号聚合器 + 中文研究摘要"，可单独开新条目立项，不沿用此条。
...
- ✅ **Editors diff-over-week 视图**（2026-04-23 落地）：`/code/[week]` Editor Updates 分区顶部新增 "Week-over-Week Overview" 矩阵视图，10 个 editor（4 IDE + 6 CLI）按 category 分组、按 activity DESC 排序，每行 4 列：name+version / 5 dot 活跃度 / theme chips / WoW delta。数据层 `computeEditorDiffMatrix()` 在 `website/lib/code-weekly.ts` 纯计算（无 LLM）：activity dots 由 `highlights.length` + `version` 非空 `+1` + stub aiSummary clamp 0 合成；6 种 theme（release/feature/fix/perf/policy/integration）由 CN+EN 关键词字典匹配；WoW 5 种（first/silent/accelerating/slowing/steady + unknown fallback）基于两周 `highlights` 数差比较。原 `EditorCard` 网格保留在矩阵下方，做"scan → 深读"两层阅读入口。首次运行 W16：steady=8 / slowing=1 / new=1 / silent=1 / accelerating=1；W15 首周（无 prev）fallback 每行 WoW 显示 `—` + legend 附"no previous week — WoW disabled"。openspec change `code-weekly-editor-diff` 已归档 + 1 个新 spec。
...
2. ✅ **Stars → AI Daily**（2026-04-23 落地）：被 AI leader 共同 star 的 repo 以 `RawNewsItem` 形态注入 AI Daily 主管线，经 DeepSeek scoring 自然进入 research/engineering section。新 source 模块 `scripts/ai-daily/sources/stars-co-starred.ts`（~155 行）按 `ai-daily-ingest.ts` 对称模式——**不跨 import** website/lib，直接读 `profile-data/github-stars/YYYY-MM-DD.json` 30 天滚动窗口，聚合 `starredBy` 去重 `size ≥ 2`，合成 `summary = "Starred by N leaders (<handles>): <description>"` 让社会证明信号对 DeepSeek prompt 可见。**ROADMAP 原门槛 ≥3 不成立**——实测全数据集 count≥3 为 0（2026-04-23 扫描：60d 窗口 33 total / count≥2 仅 2 / count≥3 = 0），采用与 Stars 板块 `computeCoStarredRepos` 默认一致的 `minCount=2`，30d 窗口稳定产出 2 条高质量候选（`huggingface/ml-intern` 7 分 / `mattmireles/gemma-tuner-multimodal` 8 分）。metrics `SourceBreakdown` 加 `coStarred: number`，`SourceStackChart` 加 lime `#a3e635` 色段 + legend。openspec change `ai-daily-co-starred-source` 已归档 + 1 个新 spec。
