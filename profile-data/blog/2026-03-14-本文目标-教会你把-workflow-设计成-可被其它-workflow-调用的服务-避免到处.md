...
readTime: "4 min read"
featured: false
image: "/blog/covers/2026-03-14-本文目标-教会你把-workflow-设计成-可被其它-workflow-调用的服务-避免到处.png"
excerpt: "本文介绍了如何将 workflow 设计成可复用的服务，通过定义多模式（mode）路由、使用 `<invoke-workflow>` 传参调用以及利用 `template-output` 返回结构化数据，从而避免在不同 workflow 中重复相同的逻辑，提升开发效率和代码可维护性。"
---
