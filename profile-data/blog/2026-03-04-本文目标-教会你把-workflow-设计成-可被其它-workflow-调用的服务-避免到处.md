...
date: "2026-03-04"
tags: ["workflow","服务化设计","代码复用","自动化","开发效率"]
category: "Engineering"
readTime: "4 min read"
featured: false
image: "/blog/covers/2026-03-04-本文目标-教会你把-workflow-设计成-可被其它-workflow-调用的服务-避免到处.png"
excerpt: "本文介绍了如何将 workflow 设计成可被其他 workflow 调用的服务，以避免重复逻辑。核心方法是创建同时支持交互模式和服务模式（通过 mode 参数路由）的 workflow，使用 `<invoke-workflow>` 和 `<param>` 进行参数传递，并通过 `template-output` 返回结构化结果供调用方使用。"
---
