...
readTime: "4 min read"
featured: false
image: "/blog/covers/2026-03-26-本文目标-教会你把-workflow-设计成-可被其它-workflow-调用的服务-避免到处.png"
excerpt: "本文介绍了如何将 workflow 设计成可复用的服务，通过定义多模式（mode）路由，使用 `<invoke-workflow>` 和 `<param>` 进行参数传递，并利用 `template-output` 返回结构化结果，从而避免逻辑重复，提升自动化流程的模块化和维护性。"
---
